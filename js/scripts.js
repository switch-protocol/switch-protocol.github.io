...
function animateText(target, duration = 1500, intervalTime = 35) {
    ...
    return newInterval;  // Return the interval ID so we can manage it later
}

// Start animation on page load
window.onload = function () {
    h1Interval = animateText(h1, 1500, 35);  // Initial load animation for h1
    h2Interval = animateText(h2, 3000, 35);  // Initial load animation for h2
    h3Interval = animateText(h3, 4500, 35);  // Initial load animation for h3
};

// Mouseover event to reset and restart animation
h1.addEventListener('mouseover', () => {
    clearInterval(h1Interval);  // Stop any ongoing animation on h1
    h1Interval = animateText(h1, 0, 35);   // Restart the animation for h1
});
